...
- `EditorSidebar` contains the Lit editing panels, split across four tabs: General (collaborators and map config), Paint (block library and texture editor), Blocks (the block library alone, at full height) and Layers. A single `block-library` element serves both tabs that show it: it lives in the sidebar light DOM and is projected into the active tab through a named slot, so its WebGL preview grid is never torn down and rebuilt. Its `layout` property (`compact` or `fill`) is what changes between the two.
