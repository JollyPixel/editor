...
await editorRuntime.load(scene, { maxFps: params.maxFps });
await scene.ready;
...
Takes the arguments of `Runtime.create`. Open dialogs and popovers keep their
...
Loads the scene without a loading screen. `load` resolves before the scene
