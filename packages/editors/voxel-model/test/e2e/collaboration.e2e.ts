...
test("a peer sees edits, selections and appears among the collaborators", async({ page, browser, model }) => {
  test.slow();
  const peerContext = await browser.newContext();
  const peer = await peerContext.newPage();

  try {
    await openEditor(peer, model, { username: "Peer" });
...
test("a block dragged by a peer is locked and follows the drag live", async({ page, browser, model }) => {
  test.slow();
  const peerContext = await browser.newContext();
  const peer = await peerContext.newPage();
