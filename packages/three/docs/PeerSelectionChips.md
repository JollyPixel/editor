...
[PeerSelectionOverlays](./PeerSelectionOverlays.md)/[PeerHighlightPass](./PeerHighlightPass.md)
...
instead of the row growing without bound. Unlike `HighlightPass` (cost scales with distinct outlined
...
is currently drawing that ring, or whether it's `PeerSelectionOverlays` or `PeerHighlightPass` driving
...
- See `examples/scripts/selection.ts` (run `npm run dev`, open `/selection.html`) - "Torus Knot" (Peer A + Peer B, preset in the "Presence" folder) is the one object with more than one selector by default, so it's the one that shows a chip row without clicking first.
