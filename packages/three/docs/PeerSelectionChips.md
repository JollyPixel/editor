...
- See `examples/scripts/selection-peer.ts` (run `npm run dev`, open `/selection-peer.html`) - "Shared Sphere" (Alice + Bob) is the one preset object with more than one selector, so it's the only one that shows a chip row.
