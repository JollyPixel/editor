...
- Opening this into a registry is a pure refactor of prior behavior for every id already in use: a `THREE.Mesh` given an unrecognized technique id still falls back to `"outline"` (via `defaultId`), and any non-mesh target still always renders `SelectionBoundingBox` (via `fallbackId`) - see [SelectionManager](./SelectionManager.md)'s own `SelectionTechnique` notes for why that fallback order matters for a per-id override that references a scene-level pipeline technique (`"coloredOutline"`) instead of a per-object one.
