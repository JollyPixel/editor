...
- Built from `THREE.EdgesGeometry`, which filters out edges below a threshold angle - very smooth/high-poly meshes (e.g. UV spheres) can still read as a busy wireframe rather than a clean silhouette. Low-poly and hard-surface geometry (boxes, cones, icosahedrons) outline cleanly. For a smooth/high-poly mesh, [ColoredOutlinePass](./ColoredOutlinePass.md)'s screen-space silhouette (the `"coloredOutline"` technique) reads cleanly regardless of poly count - see `SelectionManager`'s own `SelectionTechnique` doc for the trade-off (a scene-level pipeline, not a per-object overlay like this class).
