...
once" perf mode, like `examples/scripts/selection-stress.ts`'s "Random Selection"
(run `npm run dev`, open `/selection-stress.html`): with hundreds or thousands of
...
- Not applicable to the `"highlight"`/`"highlightJfa"` techniques - those already cost nothing extra per target (`HighlightPass`/`HighlightPassJfa`'s own `setEntries` takes the whole batch directly), so there is no per-target draw call here to merge away.
