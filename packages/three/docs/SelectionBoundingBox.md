...
- The fill mesh's material always keeps `depthWrite: false`, regardless of `xray` - a translucent fill corrupting the depth buffer for whatever draws after it is the same class of bug already fixed once in `HighlightPass`'s own priority-mask material, so it's never risked here even though nothing currently reads this fill's depth.
...
- See `examples/scripts/selection.ts` (run `npm run dev`, open `/selection.html`) - the "Cluster" group there uses this class, shown only when the group itself is selected from the outliner; clicking a part directly in the 3D view selects that mesh via [SelectionManager](./SelectionManager.md). The "group fill opacity" slider there drives `fillOpacity` live via `SelectionManager.setBoundingBoxOptions`.
