...
scene.add(cluster);
highlight.register("cluster", cluster);
selectables.add(
  {
    id: "cluster",
    label: "Cluster (group)",
...
if (outlinerPane === null) {
  throw new Error("mesh-highlight: no #outliner jolly-pane in this page's HTML");
}
...
tree.addEventListener("jolly-select", (event) => {
  highlight.select(event.detail.selected[0] ?? null);
});
