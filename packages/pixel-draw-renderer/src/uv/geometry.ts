...
export { UVFaceSet } from "./UVFaceSet.ts";
...
export function copyGeometry(
  ...
  return UVGeometryValue.from(geometry).toJSON();
}

export function rectOf(
  geometry: UVGeometry
): SelectionRect {
  return UVGeometryValue.from(geometry).bounds;
}
...
export function geometryAt(
  ...
  return UVGeometryValue.from(geometry).withBounds(rect).toJSON();
}
...
export function pointInGeometry(
  ...
  return UVGeometryValue.from(geometry).contains(pos);
}
