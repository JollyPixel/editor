...
import { LineTool } from "../../src/input/LineTool.ts";

describe("LineTool", () => {
  describe("rasterize", () => {
    test("horizontal line", () => {
      const points = LineTool.rasterize({ x: 0, y: 0 }, { x: 3, y: 0 });
    ...
    test("vertical line", () => {
      const points = LineTool.rasterize({ x: 0, y: 0 }, { x: 0, y: 3 });
      assert.deepStrictEqual(points, [
    ...
    test("45 degree diagonal", () => {
      const points = LineTool.rasterize({ x: 0, y: 0 }, { x: 3, y: 3 });
      assert.deepStrictEqual(points, [
    ...
    test("arbitrary slope stays contiguous (no diagonal gaps)", () => {
      const points = LineTool.rasterize({ x: 0, y: 0 }, { x: 5, y: 2 });
    ...
    test("works in all directions (negative deltas)", () => {
      const points = LineTool.rasterize({ x: 5, y: 5 }, { x: 2, y: 8 });
    ...
    test("zero-length segment rasterizes to a single point", () => {
      const points = LineTool.rasterize({ x: 4, y: 4 }, { x: 4, y: 4 });
