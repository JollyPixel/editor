...
/**
 * Membership bookkeeping and rights-filtered fan-out for a single room.
 * Kept free of RightsGate/Extension so it can be tested in isolation.
 */
export class RoomMembers {
  ...
  add(
    ...
    this.#members.set(clientId, record);
  }
  ...
  send(
    ...
    const { excludeClientId, predicate } = options;

    for (const [memberId, record] of this.#members) {
      if (memberId === excludeClientId) {
        continue;
      }
      if (predicate && !predicate(record.role)) {
        continue;
      }
